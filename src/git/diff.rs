...
pub trait GitDiff {
    fn git_diff(exclude_files: &[String]) -> ReviseResult<String> {
        let mut args = vec!["diff", "--staged"];

        let exclude_args: Vec<String> = exclude_files
            .iter()
            .map(|file| format!(":!{file}"))
            .collect();
        args.extend(exclude_args.iter().map(std::string::String::as_str));
...
mod tests {
    ...
    fn test_git_diff() {
        ...
        let diff =
            GitDiffImpl::git_diff(&["CHANGELOG.md".to_string()]).unwrap();
        println!("Git diff: {diff}");
    }
}
